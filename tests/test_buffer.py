...
def test_scaler_buffer():
    ...
    scaler = liter.engine.buffer.ScalarSmoother(3)
    print(scaler)
    ...
    new_scaler = liter.engine.buffer.ScalarSmoother(3)
...
def test_exponential_moving_average_buffer():
    ...
    scaler = liter.engine.buffer.ExponentialMovingAverage(1 / 3)
    scaler_ = liter.engine.buffer.ExponentialMovingAverage(window_size=3)
    ...
    new_scaler = liter.engine.buffer.ExponentialMovingAverage(0.1)
    new_scaler.load_state_dict(pickle.loads(state))
...
class SimpleClass:
    def __init__(self):
        self.buffer = {
            f"arg{idx}": liter.engine.buffer.ScalarSmoother(5) for idx in range(3)
        }

    @liter.engine.buffer.to_buffer("buffer")
    def generator(self):
        for idx in range(5):
            yield f"arg{idx}", idx
